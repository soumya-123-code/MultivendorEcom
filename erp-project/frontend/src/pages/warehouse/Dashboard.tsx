...
import { inventoryApi, purchaseOrdersApi, salesOrdersApi } from '../../api';
import { Inventory, PurchaseOrder, SalesOrder } from '../../types';
...
const WarehouseDashboard: React.FC = () => {
  ...
  const fetchDashboardData = useCallback(async () => {
    ...
    try {
      ...
      setStats({
        totalItems: inventoryRes?.data.count ?? 0,
            ...
            <CardHeader
              title={
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                  <InboundIcon color="success" /> Pending Inbound
                </Box>
              }
              action={
                <Button size="small" onClick={() => navigate('/warehouse/inbound')}>
                  View All
                </Button>
              }
            />
                  ...
                  {pendingPOs.length === 0 ? (
                    ...
                    pendingPOs.map((po) => (
                      <TableRow
                        ...
                        hover
                        sx={{ cursor: 'pointer' }}
                        onClick={() => navigate(`/warehouse/inbound/${po.id}`)}
                      >
                        ...
                        <TableCell>
                          <StatusChip status={po.status} category="purchaseOrder" />
                        </TableCell>
                      </TableRow>
                    ))
                  )}
        ...
        <Grid size={{ xs: 12, md: 6 }}>
          <Card>
            <CardHeader
              title={
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                  <ShippingIcon color="primary" /> Ready to Ship
                </Box>
              }
              action={
                <Button size="small" onClick={() => navigate('/warehouse/outbound')}>
                  View All
                </Button>
              }
            />
                  ...
                  {pendingShipments.length === 0 ? (
                    ...
                    pendingShipments.map((order) => (
                      <TableRow
                        ...
                        hover
                        sx={{ cursor: 'pointer' }}
                        onClick={() => navigate(`/warehouse/outbound/${order.id}`)}
                      >
                        <TableCell sx={{ fontWeight: 600 }}>
                          {order.order_number}
                        </TableCell>
                        ...
                        <TableCell>
                          <StatusChip status={order.status} category="order" />
                        </TableCell>
                      </TableRow>
                    ))
                  )}
          ...
          <Card>
            <CardHeader
              ...
              action={
                <Button
                  size="small"
                  onClick={() => navigate('/warehouse/stock?status=low_stock')}
                >
                  View All
                </Button>
              }
                    ...
                    <Grid key={item.id} size={{ xs: 12, sm: 6, md: 3 }}>
                      ...
                      >
                        ...
                        <LinearProgress
                          ...
                          variant="determinate"
                          value={Math.min(
                            (item.quantity / (item.reorder_level || 10)) * 100,
                            100
                          )}
                          color={item.quantity === 0 ? 'error' : 'warning'}
                        />
